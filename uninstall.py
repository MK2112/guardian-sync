...
class UninstallManager:
    ...
    def uninstall_package(self):
        try:
            ...
            if result.returncode == 0:
                ...
                self.log(
                    "Package not found in pip (may be local installation)", "DEBUG"
                )
                return True
        except Exception as e:
    ...
    def remove_entry_point(self):
        try:
            ...
            result = subprocess.run(
                ["which", "guardian-sync"], capture_output=True, text=True
            )
            if result.returncode == 0:
                ...
                self.log(
                    "Command-line tool not found (may already be removed)", "DEBUG"
                )
            return True
        except Exception as e:
    ...
    def remove_pq_keystore(self):
        ...
        try:
            ...
            if pq_keystore.exists():
                if not self.force:
                    response = input(
                        f"\nRemove post-quantum keystore at {pq_keystore}? [y/N]: "
                    )
                    if response.lower() != "y":
    ...
    def remove_auto_start(self):
        try:
            self.log("Removing auto-start configuration...")
            service_path = (
                Path.home() / ".config" / "systemd" / "user" / "guardian-sync.service"
            )
            if service_path.exists():
                subprocess.run(
                    ["systemctl", "--user", "stop", "guardian-sync.service"],
                    capture_output=True,
                )
                subprocess.run(
                    ["systemctl", "--user", "disable", "guardian-sync.service"],
                    capture_output=True,
                )
                ...
                self.success_msg("Removed auto-start service")
            else:
                self.log("Auto-start service not found", "DEBUG")
            return True
        except Exception as e:
            self.error(f"Failed to remove auto-start: {str(e)}")
            return False
    ...
    def run(self):
        ...
        # Run uninstall steps
        steps = [
            ("Removing auto-start service", self.remove_auto_start),
...
Examples:
  ...
  python uninstall.py -v              # Verbose output
    ...
    manager = UninstallManager(
        verbose=args.verbose, force=args.force, keep_config=args.keep_config
    )
