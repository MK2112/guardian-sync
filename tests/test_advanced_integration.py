...
class DummyPGPComplex:
    ...
    def encrypt_file(self, file_path, output_path=None):
        ...
        self.last_encrypted_file = file_path
...
class TestEndToEndWorkflows:
    def test_multiple_file_sync_workflow(self, tmp_path):
        ...
        assert pgp.encrypt_count == 5, f"Expected 5 encryptions, got {pgp.encrypt_count}"
...
class TestConcurrentOperations:
    def test_concurrent_file_handling_no_corruption(self, tmp_path):
        ...
        threads = [threading.Thread(target=create_and_encrypt, args=(i,)) for i in range(5)]
        for t in threads:
            t.start()
        for t in threads:
            t.join()
        ...
        # All should succeed
        assert all(r[0] == 'success' for r in results), f"Some operations failed: {results}"
