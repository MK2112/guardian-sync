...
def hybrid_pgp_handler_patched(dummy_pgp_config):
    with mock.patch('src.hybrid_pgp_handler.PGPHandler', DummyPGPHandler):
        with mock.patch('src.hybrid_pgp_handler.HybridEncryption', return_value=DummyHybridCrypto()):
...
class TestHybridPGPHandlerInitialization:
    ...
    def test_init_hybrid_mode_disabled(self, dummy_pgp_config):
        with mock.patch('src.hybrid_pgp_handler.PGPHandler', DummyPGPHandler):
            with mock.patch('src.hybrid_pgp_handler.HybridEncryption', side_effect=ImportError):
    ...
    def test_init_handles_unavailable_hybrid(self, dummy_pgp_config):
        with mock.patch('src.hybrid_pgp_handler.PGPHandler', DummyPGPHandler):
            ...
            with mock.patch('src.hybrid_pgp_handler.HybridEncryption', side_effect=raise_error):
                handler = HybridPGPHandler(dummy_pgp_config, hybrid_mode=True)
...
class TestHybridPGPHandlerKeyStorage:
    ...
    def test_pq_keystore_permissions_secure(self, dummy_pgp_config, tmp_path):
        ...
        if keystore_path.exists():
            ...
            assert mode & 0o077 == 0, f"Keystore permissions too permissive: {oct(mode)}"
...
class TestHybridPGPHandlerEncryption:
    ...
    def test_encrypt_with_explicit_output_path(self, hybrid_pgp_handler_patched, tmp_path):
        test_file = tmp_path / "plaintext.txt"
        test_file.write_text("data")
        output_file = tmp_path / "custom_encrypted.bin"
        
        result = hybrid_pgp_handler_patched.encrypt_file(str(test_file), str(output_file))
...
class TestHybridPGPHandlerDecryption:
    ...
    def test_decrypt_with_output_path(self, hybrid_pgp_handler_patched, tmp_path):
        ...
        result = hybrid_pgp_handler_patched.decrypt_file(str(encrypted_file), str(output_file))
        
        assert result == str(output_file)
        assert os.path.exists(str(output_file))
    
    def test_decrypt_with_checksum_verification(self, hybrid_pgp_handler_patched, tmp_path):
...
class TestHybridPGPHandlerPublicKeyExport:
    ...
    def test_export_pq_public_key_file_permissions(self, hybrid_pgp_handler_patched, tmp_path):
        import stat
...
class TestHybridPGPHandlerBackwardCompatibility:
    def test_hybrid_handler_works_without_pq_keys(self, dummy_pgp_config):
        with mock.patch('src.hybrid_pgp_handler.PGPHandler', DummyPGPHandler):
            with mock.patch('src.hybrid_pgp_handler.HybridEncryption', return_value=None):
                handler = HybridPGPHandler(dummy_pgp_config, hybrid_mode=True)
    ...
    def test_encrypt_fallback_to_pgp_on_hybrid_error(self, dummy_pgp_config, tmp_path):
        ...
        with mock.patch('src.hybrid_pgp_handler.PGPHandler', DummyPGPHandler):
            with mock.patch('src.hybrid_pgp_handler.HybridEncryption', return_value=mock_hybrid):
                handler = HybridPGPHandler(dummy_pgp_config, hybrid_mode=True)
                # Make hybrid encryption fail
                mock_hybrid.encrypt_hybrid = mock.Mock(side_effect=Exception("Hybrid failed"))
