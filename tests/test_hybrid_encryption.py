...
class TestHybridEncryptionKeyGeneration:
    ...
    @pytest.mark.skip(reason="liboqs is installed, testing unavailability requires complex module reloading")
...
class TestHybridEncryptionDecryption:
    def test_encrypt_decrypt_roundtrip(self, hybrid_crypto, mock_liboqs):
        ...
        # Mock the KEM for consistent shared secret in decryption
        # We need to ensure encap and decap use the same shared secret
        original_kem = mock_liboqs.kem_instance
        
        class ConsistentMockKEM:
            def __init__(self, algorithm, public_key=None):
                self.algorithm = algorithm
                self.public_key = public_key
                self.shared_secret_value = None
            
            def encap_secret(self):
                self.shared_secret_value = os.urandom(32)
                kem_ciphertext = b"KEMCT_" + os.urandom(1088)
                return kem_ciphertext, self.shared_secret_value
            
            def decap_secret(self, kem_ciphertext):
                # Return the shared secret that was used in encap
                return self.shared_secret_value
    ...
    def test_encrypt_produces_different_output(self, hybrid_crypto):
        ...
        assert encrypted1 != encrypted2, "Different nonces should produce different ciphertexts"
...
class TestHybridEncryptionSecurity:
    def test_no_deterministic_encryption(self, hybrid_crypto):
        ...
        ciphertexts = [hybrid_crypto.encrypt_hybrid(plaintext, pub_key) for _ in range(10)]
    ...
    def test_kem_ciphertext_uniqueness(self, hybrid_crypto):
        ...
        # Extract KEM ciphertext portions (after version byte)
        kem1 = encrypted1[1:1089]
        kem2 = encrypted2[1:1089]
        
        # KEM should be deterministic per public key, but nonce ensures overall difference
        assert encrypted1 != encrypted2
...
class TestHybridEncryptionEdgeCases:
    def test_encrypt_with_invalid_key_format(self, hybrid_crypto):
        ...
        # Mock KEM to fail on invalid key
        original_kem = hybrid_crypto.hybrid_crypto.KeyEncapsulation if hasattr(hybrid_crypto, 'hybrid_crypto') else None
