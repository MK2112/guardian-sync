...
class HybridPGPHandler:
    ...
    def _load_or_generate_pq_keys(self):
        # Load post-quantum keypair from storage or generate new ones
        ...
        if os.path.exists(keystore_path):
            try:
                ...
                self.logger.warning(f"Failed to load PQ keypair: {str(e)}. Generating new one.")
        
        # Generate new keypair
        if self.hybrid_crypto:
            try:
                self.pq_public_key, self.pq_secret_key = self.hybrid_crypto.generate_keypair()
    ...
    def _save_pq_keys(self, keystore_path: str):
        # Securely save post-quantum keypair to storage
        try:
            data = {
    ...
    def encrypt_file(self, file_path: str, output_path: Optional[str] = None, use_hybrid: bool = True) -> str:
        ...
        if use_hybrid and self.hybrid_mode and self.pq_public_key:
            try:
                ...
                hybrid_ciphertext = self.hybrid_crypto.encrypt_hybrid(pgp_ciphertext, self.pq_public_key)
                
                # Write hybrid encrypted file
                with open(output_path, 'wb') as f:
                    f.write(hybrid_ciphertext)
                
                self.logger.info(f"Encrypted {file_path} to {output_path} (hybrid PGP + ML-KEM)")
                return output_path
                ...
                self.logger.error(f"Hybrid encryption layer failed: {str(e)}. Using PGP only.")
                return pgp_encrypted
        
        return pgp_encrypted
    
    def decrypt_file(self, encrypted_path: str, output_path: Optional[str] = None, verify_with: Optional[str] = None) -> str:
        ...
        if is_hybrid and self.pq_secret_key:
            try:
                ...
                pgp_ciphertext = self.hybrid_crypto.decrypt_hybrid(data, self.pq_secret_key)
                import tempfile
                ...
                try:
                    result = self.pgp_handler.decrypt_file(temp_path, output_path, verify_with)
                    self.logger.info(f"Decrypted {encrypted_path} (hybrid PGP + ML-KEM)")
                    return result
                finally:
                ...
                self.logger.warning(f"Hybrid decryption failed: {str(e)}. Falling back to PGP-only.")
